...
if(isset($_GET['add'])){
	...
	while($row = fetch_array($query)){
		if ($row['product_quantity']!= $_SESSION['product_' . $_GET['add']]){
			...
			redirect("../public".DS."checkout.php");
		}
		else{
			set_message("WE only have " . $row['product_quantity'] . " "."{$row['product_title']}"." Availble");
			redirect("../public".DS."checkout.php");
		}
	}
}
...
function process_transaction(){
	...
	foreach( $_SESSION as $name => $value){
		...
		if (substr($name, 0, 8) == "product_"){
			...
			$send_query = query("INSERT INTO orders (order_amount, order_transaction, order_currency, order_status ) VALUES('{$amount}', '{$transaction}','{$currency}','{$status}')");
